...
def lambda_handler(event, context):
    task = sf.get_activity_task(activityArn=activity, workerName="new-listings-bucketing-app")
    logger.info(f"Task: {task}")
    cluster_id = emr_client.run_job_flow(
        Name='new-listings-bucketing-app-job',
        ...
        Steps=[{
                'Name': 'New Listings Bucketing App',
                ...
                'HadoopJarStep': {
                    ...
                    'Args': [
                        "bash",
                        "-c",
                        f"""aws s3 cp s3://rdc-recommended-notifications-{env}/rn_nl_bucketing_app_driver.sh .;
                        chmod 755 rn_nl_bucketing_app_driver.sh;
                        ./rn_nl_bucketing_app_driver.sh {task['taskToken']} {env};
                        rm rn_nl_bucketing_app_driver.sh"""
                    ]
                }
            }
        ],
