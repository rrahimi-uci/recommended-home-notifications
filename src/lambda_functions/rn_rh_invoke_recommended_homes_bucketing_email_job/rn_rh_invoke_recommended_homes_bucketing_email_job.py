...
def lambda_handler(event, context):
    task = sf.get_activity_task(activityArn=activity, workerName="recommended-homes-bucketing-email")
    logger.info(f"Task: {task}")
    cluster_id = emr_client.run_job_flow(
        Name='recommended-homes-bucketing-email-job',
        ...
        Steps=[{
                'Name': 'Recommended Homes Bucketing Email',
                ...
                'HadoopJarStep': {
                    ...
                    'Args': [
                        "bash",
                        "-c",
                        f"""aws s3 cp s3://rdc-recommended-notifications-{env}/rn_rh_bucketing_email_driver.sh .;
                        chmod 755 rn_rh_bucketing_email_driver.sh;
                        ./rn_rh_bucketing_email_driver.sh {task['taskToken']} {env};
                        rm rn_rh_bucketing_email_driver.sh"""
                    ]
                }
            }
        ],
