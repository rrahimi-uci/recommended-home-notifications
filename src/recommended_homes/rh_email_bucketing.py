...
def driver(args):
    """
    Driver method to bucket Recommended Homes users for Email
    :return: None
    """
    try:
        ...
        optimizely_variation.bucketing_users_email_V1(args.env, spark, target_date)
        gc.collect()
        log.info("Candidates with Recommended Homes Email variation uploaded to S3!")

        # send success token to step function activity
        response = sf.send_task_success(taskToken=args.task_token,
                                        output=json.dumps(
                                            {'message': 'Recommended Homes Email Bucketing Completed!'}))

    except Exception as e:
        _ = sf.send_task_failure(taskToken=args.task_token)
        exc_type, exc_obj, exc_tb = sys.exc_info()
        f_name = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        log.error("Filename = {} || Line Number = {} || Recommended Homes Email Bucketing Driver ERROR: {}".format(
