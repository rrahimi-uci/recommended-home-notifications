...
def driver(args):
    """
    Driver method to bucket New Listings users for Native App
    :return: None
    """
    try:
        ...
        eligible_users = eligiblilty_filtering.Eligibility(args.env, target_date, spark, None,
                                                           None, None, None, None)
        eligible_users.get_app_push_eligible_users()
        gc.collect()
        log.info("App DomZip Eligible Users filtering Done!")

        # get experiment variation of eligible users
        optimizely_variation.bucketing_users_app_domzip(args.env, spark, target_date)
        gc.collect()
        
        optimizely_variation.bucketing_users_app_v2(args.env, spark, target_date)
        gc.collect()
        log.info("Candidates with New Listings App variation uploaded to S3!")

        # send success token to step function activity
        response = sf.send_task_success(taskToken=args.task_token,
                                        output=json.dumps(
                                            {'message': 'New Listings App Bucketing Completed!'}))

    except Exception as e:
        _ = sf.send_task_failure(taskToken=args.task_token)
        exc_type, exc_obj, exc_tb = sys.exc_info()
        f_name = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        log.error("Filename = {} || Line Number = {} || New Listings App Bucketing Driver ERROR: {}".format(
