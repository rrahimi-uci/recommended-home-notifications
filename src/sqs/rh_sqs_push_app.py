...
def driver():
    try:
        ...
        sqs = sqs_push.SQS(args['ENV'], queue, "recommended_homes_app", spark, target_date, recommended_homes_base_path,
        ...
        response = sf.send_task_success(taskToken=args['task_token'], output=json.dumps(
            {'message': 'Recommended Homes SQS Push for Native App completed'}))
        log.info("Pushed all the recommendations for Native App to SQS!")

    except Exception as e:
        response = sf.send_task_failure(taskToken=args['task_token'])
        log.error("Recommended Homes SQS Push for Native App ERROR: {}".format(e.__str__()))
