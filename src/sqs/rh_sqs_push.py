...
def driver():
    try:
        ...
        sqs = sqs_push.SQS(args['ENV'], queue, "recommended_homes", spark, target_date, recommended_homes_base_path,
        ...
        response = sf.send_task_success(taskToken=args['task_token'], output=json.dumps(
            {'message': 'Recommended Homes SQS Push completed'}))
        log.info("Pushed all the recommendations to SQS!")

    except Exception as e:
        response = sf.send_task_failure(taskToken=args['task_token'])
        log.error("Recommended Homes SQS Push ERROR: {}".format(e.__str__()))
