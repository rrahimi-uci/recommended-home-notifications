...
def driver():
    try:
        ...
        sqs = sqs_push.SQS(args['ENV'], queue, "new_listings_app", spark, target_date, new_listings_base_path,
                           new_listings_base_local_path)
        sqs.push_recommendations()

        domzip_sqs = sqs_push.SQS(args['ENV'], queue, "dom_zip_app", spark, target_date, None, None)
        domzip_sqs.push_dom_zip_notifications()

        response = sf.send_task_success(taskToken=args['task_token'], output=json.dumps(
            {'message': 'New Listings Recommended Notifications SQS Push for Native App completed'}))
        log.info("Pushed all the new listings recommendations for Native App to SQS!")

    except Exception as e:
        _ = sf.send_task_failure(taskToken=args['task_token'])
        exc_type, exc_obj, exc_tb = sys.exc_info()
        f_name = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        log.error("Filename = {} || Line Number = {} || New Listings SQS Push for Native App ERROR: {}".format(
