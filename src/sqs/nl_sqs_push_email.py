...
def driver():
    try:
        ...
        sqs = sqs_push.SQS(args['ENV'], queue, "new_listings_email", spark, target_date, new_listings_base_path,
        ...
        response = sf.send_task_success(taskToken=args['task_token'], output=json.dumps(
            {'message': 'New Listings Recommended Notifications SQS Push for Email Campaign completed'}))
        log.info("Pushed all the new listings recommendations for Email Campaign to SQS!")

    except Exception as e:
        _ = sf.send_task_failure(taskToken=args['task_token'])
        exc_type, exc_obj, exc_tb = sys.exc_info()
        f_name = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        log.error("Filename = {} || Line Number = {} || New Listings SQS Push for Email Campaign ERROR: {}".format(
